...
- Themes: all colors live as CSS custom properties at the top of
  ...
  query. See `neon90s` and `midnight` for the two patterns. Two rules: theme
  blocks change look only, never layout properties the mobile media query
  touches, and if you change a base structural rule, mirror it in every
  ...
  the default theme's colors, since it's a static file.)
